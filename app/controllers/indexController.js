...
const advanceSettlementSchema = Joi.object().keys({
  venueBillNo: Joi.string()
    .trim()
    .required(),
  betTime: Joi.string()
    .trim()
...
async function createItem(ctx, next) {
  ...
  let body = await Joi.validate(ctx.request.body, userSchema, {
    allowUnknown: true
  });
...
async function createItem2(ctx, next) {
  // Joi validation, more info: https://github.com/hapijs/joi
